...
val configModule = module {
    single<DataSource> { DataSourceConfig()
        .dataSource(
    ...
    single<Flyway> { FlywayConfig().flyway(getProperty("flyway.locations"), get()) }
    single<MigrateResult>(createdAtStart = true) { FlywayConfig().migrate(get()) }

    single<DSLContext> { JooqConfig().dslContext(get()) }
}
